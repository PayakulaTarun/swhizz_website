...
const FAQ: React.FC = () => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: "We provide 100% Placement Support—which includes elite resume branding, intensive interview preparation, and direct job referrals through our network of 200+ hiring partners until you get hired."
    },
    {
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <div className="text-center mb-16 reveal">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-8 max-w-xl mx-auto font-light text-lg">
            Find the answers you need to start your elite transition into the global IT sector.
          </p>
        </div>
          ...
          {faqs.map((faq, i) => (
            ...
            >
              ...
              >
                <div className="flex items-center gap-6">
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                    openIndex === i ? 'bg-teal-500 text-white rotate-12' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 group-hover:bg-teal-50 dark:group-hover:bg-teal-900/20 group-hover:text-teal-500'
                  }`}>
                    <HelpCircle className="w-5 h-5" />
                  </div>
                  <span className={`text-lg sm:text-xl font-bold transition-colors ${
                    openIndex === i ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'
                  }`}>
                    {faq.q}
                  </span>
                </div>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 ${
                  openIndex === i ? 'bg-teal-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'
                }`}>
              ...
              <div className={`transition-all duration-700 ease-in-out ${
                openIndex === i ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'
              }`}>
